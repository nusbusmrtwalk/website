...
<Collapsible client:idle title={"How does this work on the other lines?"}>
  On a linear line, like the East-West Line, end stations can usually pull
  double duty as both the local and <Indicator g />. Tuas Link is always going
  to be west of any other station. But for the Circle Line, there is no end
  station along the main loop\! Hence, there is a <Indicator g /> that’s
  distinct from the local one. This <Indicator g /> is useful as a secondary
  piece of information — it can help you confirm you’ve boarded the right train
  even after you leave the station.
</Collapsible>
...
<Image
  src={cclPlatDiagram}
  class="wide"
  alt="Diagram showing platform at Paya Lebar station, highlighting the platform labels and full line maps"
/>
...
Finally, the third key touchpoint is the screens, also known by its technical name Station Travel Information System (**STIS**). These screens primarily deliver the <Indicator s short /> and estimated wait time of arriving trains. However, as a legacy system, the STIS we have is subject to technical limitations — even as a dynamic display, it is an aged software with rigid structures, meaning it can be hard to change the layout too much from what you see on other lines.

In a signage system that works coherently, each touchpoint can have a clear primary responsibility. That does not mean it delivers only one piece of information — each touchpoint can be designed to subtly reinforce each other, layering information so the passenger builds up a complete picture after passing by them one by one. A passenger entering the station concourse might first look at the platform labels on hanging signs to head to the right platform, look above the platform screen doors for the platform label and PRD for confirmation that they are heading the right way, and finally refer to the STIS screen to determine if they should board or skip the arriving train.
...
<Collapsible
  client:idle
  title={"How many PRDs are incorrectly oriented, actually?"}
>
  As of 10 July 2026, of the 33 stations on the Circle Line, only two have both
  platforms’ PRDs correctly oriented: Kent Ridge and Prince Edward Road. The
  majority of stations (28) have one PRD mirrored such that both PRDs curve the
  same way, while *three* stations have both platforms’ PRDs mirrored. The
  vertical SRDs are always correctly curved, but each platform typically only
  has one SRD signage whereas it will have five to six PRDs. The PRD is much
  more likely to be seen by a rushing passenger.
</Collapsible>
...
<Collapsible client:idle title={"Why is a global direction identifier useful?"}>
  When giving directions to a friend and you know you are taking the train in
  the same direction, you can use the <Indicator g /> to agree on the train to
  take. If you need to tell people which way you’re going (e.g. you’re reporting
  an incident in an emergency and the train is still moving) you can always
  report the global direction. Finally, if a train service disruption only
  affects one direction, the train operator can simply tell people what that
  direction is using the <Indicator g />.
</Collapsible>
...
Don’t get me wrong: it is useful to have space on the STIS screen for global direction, since when it is _not_ business as usual, there must be a cue to passengers to disregard the physical signage. But the STIS screens should not fundamentally be doing a job that could be done by physical signage, especially when it is already too small — currently, Circle Line stations only have one STIS screen per platform, even in the new Stage 6 stations.
...
The original layout actually placed the <Indicator g /> inside the <Indicator s short />, but gave it lots of emphasis.
...
- Loop trains had a clear _positive_ identification through the word “Loop”.
...
The STIS was conveying the <Indicator g /> well, compensating for possible commuter confusion from the mirrored PRDs, but at the expense of its primary job — communicating the <Indicator s short />.
...
- Emphasis is shifted from the <Indicator g /> to (part of) the <Indicator s short />.
...
- Terminating trains are now _positively_ identified by the words “ends at”, which is a clear improvement.
...
![](../../assets/blog/2026/ccl6-stis-pyl.jpg)
...
With the old STIS screen design, the passenger would see the loop label in the <Indicator s /> “Clockwise Loop”, and realise that the train does not head towards the spur. But in the new design, there is zero positive identification on the screen, and passengers are left to remember that the _absence_ of text implies a different meaning.

**Example 2: Misidentification of shortest path**

![](../../assets/blog/2026/ccl6-stis-pmn.jpg)

Another passenger at Promenade station wants to head Clockwise to Prince Edward Road station; with the city centre crowd and the STIS being at the far end of the platform, they miss the other signages but see a train coming in with the <Indicator s /> “ends at Prince Edward Road” at a platform with a clockwise-looking route map. Without thinking further, they board the train, only to find out that the train is actually going Anticlockwise which is the long way round.

The new signage leaves the <Indicator g /> to the STIS, at a very small font size that is virtually impossible to see from afar. The <Indicator s /> “ends at \[Last Station\]” can look like a <Indicator l />, implying that the train is the fastest way to go to that station. Additionally, nothing on the platform signage would suggest to the passenger that trains on this platform only go Anticlockwise (notwithstanding any service disruptions).
...
> Update (15 July 2026): Added images elaborating issues with the current STIS screens, and additional information on the order of touchpoints a passenger might encounter when entering the station concourse.

> Update (16 July 2026): Added more service pattern colored labels for consistency.
